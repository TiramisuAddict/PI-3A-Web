...
class TaskNotificationService
{
    ...
    /**
     * Envoie un rappel de date limite proche.
     */
    public function notifyDeadlineApproaching(Tache $tache, int $daysRemaining): void
    {
        $employe = $tache->getEmploye();
        if ($employe === null || empty($employe->getEmail())) {
            return;
        ...
        $urgencyColor = $daysRemaining <= 1 ? '#ef4444' : ($daysRemaining <= 3 ? '#f59e0b' : '#4A5DEF');
        $urgencyLabel = $daysRemaining <= 0 ? 'Echeance aujourd\'hui !' : 'Echeance dans ' . $daysRemaining . ' jour' . ($daysRemaining > 1 ? 's' : '');

        $email = (new Email())
            ->from($this->senderEmail)
            ->to($employe->getEmail())
            ->subject('Rappel : ' . $urgencyLabel . ' — ' . $tache->getTitre())
            ->html(
                '<div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;">'
                . '<div style="background:' . $urgencyColor . ';color:#fff;padding:20px 24px;border-radius:8px 8px 0 0;">'
                . '<h2 style="margin:0;font-size:18px;">' . htmlspecialchars($urgencyLabel) . '</h2>'
                . '</div>'
                . '<div style="background:#f8fafc;padding:24px;border:1px solid #e2e8f0;border-top:none;border-radius:0 0 8px 8px;">'
                . '<p>Bonjour <strong>' . htmlspecialchars($employe->getPrenom() . ' ' . $employe->getNom()) . '</strong>,</p>'
                . '<p>Ceci est un rappel pour votre tache dont l\'echeance approche :</p>'
                . '<table style="width:100%;border-collapse:collapse;margin:16px 0;">'
                . '<tr><td style="padding:8px 12px;background:#fff;border:1px solid #e2e8f0;font-weight:600;width:140px;">Tache</td>'
                . '<td style="padding:8px 12px;background:#fff;border:1px solid #e2e8f0;">' . htmlspecialchars($tache->getTitre()) . '</td></tr>'
                . '<tr><td style="padding:8px 12px;background:#fff;border:1px solid #e2e8f0;font-weight:600;">Projet</td>'
                . '<td style="padding:8px 12px;background:#fff;border:1px solid #e2e8f0;">' . htmlspecialchars($projetNom) . '</td></tr>'
                . '<tr><td style="padding:8px 12px;background:#fff;border:1px solid #e2e8f0;font-weight:600;">Progression</td>'
                . '<td style="padding:8px 12px;background:#fff;border:1px solid #e2e8f0;">' . ($tache->getProgression() ?? 0) . '%</td></tr>'
                . '<tr><td style="padding:8px 12px;background:#fff;border:1px solid #e2e8f0;font-weight:600;">Date limite</td>'
                . '<td style="padding:8px 12px;background:#fff;border:1px solid #e2e8f0;font-weight:700;color:' . $urgencyColor . ';">' . $dateLimite . '</td></tr>'
                . '</table>'
                . '<p style="color:#64748b;font-size:13px;">Pensez a mettre a jour votre progression sur la plateforme.</p>'
                . '</div>'
                . '</div>'
            );
        ...
        try {
            ...
            $this->logger->error('Echec envoi email rappel deadline: ' . $e->getMessage());
        }
    }
}
